...
//     println!("type: {}, data: {:x?}", std::any::type_name::<T>(), data_bytes);
...
register_packets! {
    ...
    pub async fn request_status_update(player: &mut Player, reader: &mut Reader) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        ...
        let packet = packets::ChoUserStats::new(player.id.unwrap(), 0, "", "", 0, 0, 0, 727, 69.420, 727, 727, 727, 727 as i16).await;
